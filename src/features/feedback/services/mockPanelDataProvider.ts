import type { PanelDataProvider, PanelState, Unsubscribe } from "../../../shared/types/panelState";
...
export class MockPanelDataProvider implements PanelDataProvider {
  ...
  start(): void {
    ...
    this.envTimer = setInterval(() => {
      this.state.temperatureC = round1(clamp(this.state.temperatureC + (Math.random() - 0.5) * 0.4, 18, 32));
      this.state.humidityPct = round1(clamp(this.state.humidityPct + (Math.random() - 0.5) * 1.2, 30, 95));
      this.emit();
    }, 10000);
